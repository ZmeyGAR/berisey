const custom_block = {
          ...
          if (json['redirect']) {
            ...
            if (json['error']['warning']) {
                iziToast.warning({
                  ...
                  target: '#custom-customer .alert-target',
                  targetFirst: false,
                });
            }
        ...
        success: function(json) {
          ...
          if (json['redirect']) {
            ...
            if (json['error']['warning']) {
              iziToast.warning({
                message: json['error']['warning'],
                target: '#custom-shipping .alert-target',
                targetFirst: false,
              });
            }
        ...
        success: function(json) {
          ...
          if (json['redirect']) {
            ...
            if (json['error']['payment_method']) {
              iziToast.warning({
                message: json['error']['payment_method'],
            ...
            if (json['error']['agree']) {
              iziToast.warning({
                message: json['error']['agree'],
                target: '#custom-control .alert-target',
                targetFirst: false,
              });
              reject('control');
            }
        ...
        success: function(json) {
          ...
          if (json['error']) {
            if (json['error']['warning']) {
              iziToast.warning({
                message: json['error']['warning'],
                target: '#custom-comment .alert-target',
                targetFirst: false,
              });
  ...
  function failureCallback(id) {
    console.log('failure #custom-' + id)
    $('html, body').animate({ scrollTop: $('#custom-' + id + ' .alert-target').offset().top - ($(window).height() / 2)}, 'slow');
  }
