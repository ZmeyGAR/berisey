...
class ControllerExtensionModuleCustom extends Controller {
	public function index() {
		...
		$this->document->addScript('catalog/view/theme/berisey/javascript/custom/checkout.js?' . rand(1,10));
		...
		foreach(range(1,6) as $step){
			$data['_text_step_' . $step] = sprintf($this->language->get('_text_step'), $step);
		}
		...
		$data['back_to_shopping_action'] = $this->url->link('product/category');
