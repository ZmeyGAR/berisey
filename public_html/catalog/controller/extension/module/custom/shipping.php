...
class ControllerExtensionModuleCustomShipping extends Controller {
	public function index($setting) {
		...
		if (isset($setting['status']) && (bool)$setting['status'] === true && $this->cart->hasShipping()) {
			...
			$sortFields = ['country_id' => 1,'zone_id' => 2,'city' => 3,'postcode' => 4,'company' => 5,'address_1' => 6,'address_2' => 7,];
			usort($setting['fields'], function($a, $b) use($sortFields){
				$aPriority = $sortFields[$a['name']] ?? PHP_INT_MAX;
				$bPriority = $sortFields[$b['name']] ?? PHP_INT_MAX;
